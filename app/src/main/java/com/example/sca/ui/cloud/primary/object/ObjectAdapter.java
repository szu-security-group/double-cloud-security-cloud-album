...
import com.example.sca.ui.cloud.primary.common.base.BaseAbstractAdapter;
...
public class ObjectAdapter extends BaseAbstractAdapter<ObjectEntity> {
    ...
    protected void inflate(final ObjectEntity entity, int position) {
        if(entity.getType()==0){//文件夹
            ...
            findViewById(R.id.tv_display).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if(listener!=null){
                        listener.onDisplay(entity);
                    }
                }
            });
    ...
    public interface OnObjectListener {
        ...
        void onDisplay(ObjectEntity object);

        void onDelete(ObjectEntity object);

        void onShare(ObjectEntity object);
    }
}
